(() => {
    ...
    var t = {
            151: (t, e) => {
                function i(t, e) { if (void 0 === t) throw new Error("".concat(null != e ? e : "Value", " is undefined")); return t }

                function r(t, e) { if (null === t) throw new Error("".concat(null != e ? e : "Value", " is null")); return t } e.ensureNotNull = void 0, e.ensureNotNull = r
            }
        },
        e = {};

    function i(r) { var o = e[r]; if (void 0 !== o) return o.exports; var n = e[r] = { exports: {} }; return t[r](n, n.exports, i), n.exports }(() => {
        var t = i(151);
        ...
        var o, n;
        ! function(t) { let e;! function(t) { t.SetSymbol = "set-symbol", t.SetInterval = "set-interval" }(e = t.Names || (t.Names = {})) }(o || (o = {})),
        function(t) { let e;! function(t) { t.SymbolClick = "tv-widget-symbol-click", t.WidgetLoad = "tv-widget-load", t.WidgetReady = "tv-widget-ready", t.ResizeIframe = "tv-widget-resize-iframe", t.NoData = "tv-widget-no-data" }(e = t.Names || (t.Names = {})) }(n || (n = {}));
        const s = "__FAIL__",
            a = "__NHTTP__",
            l = new RegExp("^http(s)?:(//)?");

        function c(t = location.href) { const e = function(t) { try { const e = new URL(t); return l.test(e.protocol) ? null : a } catch (t) { return s } }(t); return e || t.replace(l, "") }
        ...
        new class extends class {
            ...
            hasCopyright() { return !!this._copyrightContainer } get widgetId() { throw new Error("Method must be overridden") } widgetUtmName() { return this.widgetId } get defaultSettings() { return r[this.widgetId] } get propertiesToWorkWith() { return [] } get useParamsForConnectSocket() { return !1 } get useSnowplowPageView() { return !1 } filterRawSettings(t) {
                const e = {},
                    i = Object.keys(t),
                    r = new Set(this.propertiesToWorkWith);
                return i.forEach((i => { r.has(i) && (e[i] = t[i]) })), e
            }
            get shouldListenToIframeResize() { return !0 } get propertiesToSkipInHash() { return ["customer", "locale"] } get propertiesToAddToGetParams() { return ["locale"] } _defaultWidth() {} _defaultHeight() {} _getScriptInfo() { const t = document.currentScript; if (!t || !t.src) return console.error("Could not self-replace the script, widget embedding has been aborted"), null; return { scriptURL: function(t) { const e = new URL(t, document.baseURI); return { host: e.host, pathname: e.pathname, href: e.href, protocol: e.protocol } }(t.src), scriptElement: t, id: t.id, rawSettings: this._scriptContentToJSON(t), overrideHost: t.getAttribute("override-host") } } _replaceScript(i) {
                const { scriptURL: r, scriptElement: o, rawSettings: s, id: a, overrideHost: l } = i, c = l || function(t) { if ("BATTLE" === g) { if (window.WIDGET_HOST) return window.WIDGET_HOST; if (t.host.match(/\.\wst\w*\.\wv$/i)) return `https://${t.host.replace(/\.(\w)v$/i,((t,e)=>`-widget.${e}v`))}` } return d }(r), h = o.parentNode;
                ...
                this.iframeContainer = h && f ? h : document.createElement("div"), s && (this.settings = this.filterRawSettings(s)), s && this._validateSettings() || (console.error("Invalid settings provided, fall back to defaults"), this.settings = this.filterRawSettings(this.defaultSettings));
                const w = "32px",
                ...
                if (I ? ((0, t.ensureNotNull)(I.parentElement).replaceChild(this.iframe, I), null == o || o.remove()) : f ? (this.iframeContainer.appendChild(this.iframe), null == o || o.remove()) : (this.iframeContainer.appendChild(this.iframe), h.replaceChild(this.iframeContainer, (0, t.ensureNotNull)(o))), this.shouldListenToIframeResize && function(t, e, i) {
                        const r = e.contentWindow;
                        if (!r) return console.error("Cannot listen to the event from the provided iframe, contentWindow is not available"), () => {};

                        function o(e) { e.source && e.source === r && e.data && e.data.name && e.data.name === t && i(e.data.data) } window.addEventListener("message", o, !1)
                    }(n.Names.ResizeIframe, this.iframe, (t => { t.width && (this.iframe.style.width = t.width + "px", this.iframeContainer.style.width = t.width + "px"), this.iframe.style.height = t.height + "px", this.iframeContainer.style.height = t.height + (this.hasCopyright() ? 32 : 0) + "px" })), m) {
            ...
            _validateSettings() {
                const t = (t, e) => { if (void 0 === t) return e; const i = String(t); return /^\d+$/.test(i) ? parseInt(i) : /^(\d+%|auto)$/.test(i) ? i : null },
            ...
            _setSettingsQueryString(t) {
                const e = this.propertiesToAddToGetParams.filter((t => -1 !== h.indexOf(t))),
                    i = function(t, e) { const i = Object.create(Object.getPrototypeOf(t)); for (const r of e) Object.prototype.hasOwnProperty.call(t, r) && (i[r] = t[r]); return i }(this.settings, e);
                for (const [e, r] of Object.entries(i)) t.searchParams.append(e, r)
            }
            _setHashString(t, e) {
                const i = {};
                e && (i.frameElementId = e), Object.keys(this.settings).forEach((t => {-1 === this.propertiesToSkipInHash.indexOf(t) && (i[t] = this.settings[t]) })), (this.useParamsForConnectSocket || this.useSnowplowPageView) && (i["page-uri"] = c());
                Object.keys(i).length > 0 && (t.hash = encodeURIComponent(JSON.stringify(i)))
            }
            _scriptContentToJSON(t) { const e = t.innerHTML.trim(); try { return JSON.parse(e) } catch (t) { return console.error(`Widget settings parse error: ${t}`), null } } _createIframe(t, e, i, r, o) {
            ...
            _addCSPErrorListener(t) { document.addEventListener("securitypolicyviolation", (e => { e.blockedURI.includes(t) && (this._tryFixCSPIssueWithFallback(t), console.warn("Please update your CSP rules to allow the tradingview-widget.com origin for frame-src.")) })) } _tryFixCSPIssueWithFallback(t) {
            ...
            _updateCopyrightHrefParams(t, e, i) {
                if (this._copyrightContainer) {
                    ...
                    if (r) {
                        ...
                        } catch (t) { console.log(`Cannot update link UTM params, href="${o}"`) }
                    }
                }
            }
        } {
            get widgetId() { return "market-overview" } get useParamsForConnectSocket() { return !0 } get propertiesToWorkWith() {
